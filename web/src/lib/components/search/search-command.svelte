<script lang="ts">
	...
	// Both operands read via variables — see the search effect for why
	// short-circuiting matters with signals.
	...
	$effect(() => {
		const q = query;
		// Read BOTH filter signals unconditionally: `a || b` short-circuits,
		// and an unread signal is untracked — with a category selected, tag
		// clicks would stop re-running this effect (and vice versa).
		const hasCategory = Boolean(selectedCategory);
		const hasTags = selectedTags.length > 0;
		const hasFilters = hasCategory || hasTags;
